...
namespace Drupal\webform_verify_email\Plugin\WebformHandler;
...
class ValidateUserByOtp extends WebformHandlerBase {
  ...
  public function alterForm(array &$form, FormStateInterface $form_state, WebformSubmissionInterface $webform_submission) {
    ...
    $this->webform->getElement($this->configuration['field_name'])['#type'] == 'email') {
      $form['#attached']['library'][] = 'webform_verify_email/verify_otp';
  ...
  public function sendOtp(array &$form, FormStateInterface $form_state) {
    ...
    if (!$this->request->getSession()->get('otp_verified')) {
      if (array_intersect($user->getRoles(), $roles)) {
        ...
        $otp_form = $this->formBuilder->getForm('Drupal\webform_verify_email\Form\OtpValidatorForm');
        $otp_form['#attributes'] = [
  ...
  protected function sendOtpMail(int $otp, array $data, string $email, int $timestamp, SessionInterface $session) {
    try {
      $this->mailManager->mail("webform_verify_email", "otp_validation", $email, 'en', $data, NULL, TRUE);
      $session->set('otp', $otp);
      ...
      if ($this->configuration['debug']) {
        $this->loggerFactory->get('webform_verify_email')->notice(
      ...
      $this->loggerFactory->get('webform_verify_email')->notice($e->getMessage());
      return FALSE;
    }
  }
